var fs = require('fs'),
    ...
    models = require('next-transit-models'),
    timer = require('../timer'),
...
function import_route_extras(file_name, columns, write_path, custom_timer) {
  return new promise(function(resolve, reject) {
    models.directions.generate_directions(options.agency.id).then(function(new_directions) {
      ...
      write_data(file_name, new_directions, write_path, columns, custom_timer).then(function(count) {
        models.directions.import(options.agency.id, columns, write_path, resolve, reject);
      }, reject);
    }, reject);
  });
}
...
function import_route_shapes(file_name, columns, write_path, custom_timer) {
  return new promise(function(resolve, reject) {
    models.simplified_shapes.generate_route_shapes(options.agency.id, options.verbose).then(function(new_shapes) {
      ...
      write_data(file_name, new_shapes, write_path, columns, custom_timer).then(function(count) {
        models.simplified_shapes.import(options.agency.id, columns, write_path, resolve, reject);
      }, reject);
    }, reject);
  });
}
...
function import_simplified_stops(file_name, columns, write_path, custom_timer) {
  return new promise(function(resolve, reject) {
    models.simplified_stops.generate_stops(options.agency.id).then(function(new_simplified_stops) {
      custom_timer.interval('Time spent reading source file', true).start();
      write_data(file_name, new_simplified_stops, write_path, columns, custom_timer).then(function(count) {
        models.simplified_stops.import(options.agency.id, columns, write_path, resolve, reject);
      }, reject);
    }, reject);
  });
}
...
function import_trip_variants(file_name, columns, write_path, custom_timer) {
  return new promise(function(resolve, reject) {
    models.trip_variants.generate_variants(options.agency.id).then(function(new_trip_variants) {
      custom_timer.interval('Time spent reading source file', true).start();
      write_data(file_name, new_trip_variants, write_path, columns, custom_timer).then(function(count) {
        models.trip_variants.import(options.agency.id, columns, write_path, resolve, reject);
      }, reject);
    }, reject);
  });
}
...
function generate_stats(file_name, columns, write_path, custom_timer, total_timer) {
  return new promise(function(resolve, reject) {
    ...
    promise.all(promises).then(function(model_counts) {
      ...
      models.stats.insert(stats_data, resolve, reject);
    }, reject);
  });
}
