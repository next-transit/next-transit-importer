...
function import_agency(config, options, agency) {
  return new promise(function(resolve, reject) {
    var gtfs_path = options.gtfs_base_path + agency.slug,
        ...
        custom_importer = importers.custom(models, importer_options),
        total_timer = timer();
    ...
    sequential
      ...
      .add(add_type('Route Directions', 'directions', 'import_route_directions'))
...
function run(config, options) {
  return new promise(function(resolve, reject) {
    models.agencies.select({ public:false }, 'wat').where('slug = ?', [options.agency]).first(function(agency) {
      if(agency) {
